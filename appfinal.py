#!/usr/bin/env python3
"""
Charge Variants Tool — FFT Powering + Moment Matching (Dynamic FFT size, strict truncation)
- Algorithm dropdown allows selecting FFT Powering or Moment Matching.
- Strict truncation: displays only probabilities for charges -5..+5 (or actual computed extremes if outside)
...
*Popping error message for missing fields and sum of charges not exactly 1
...
APP_TITLE = "Charge Variants Tool — FFT + Moment Matching (Offline)"
...
class App(tk.Tk):
    ...
    def action_compute(self) -> None:
        ...
        if not self._validate_rows_strict():
            self._status("Cannot compute: missing fields detected.")
            return
